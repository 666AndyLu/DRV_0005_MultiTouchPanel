				  ...
				  const struct i2c_device_id *id)
{
	...
	//set_bit(INPUT_PROP_DIRECT, ts_dev->propbit);增加这一项的话在android里可以不用配置idc文件
	...
	request_irq(MTP_IRQ, mtp_interrupt,IRQ_TYPE_EDGE_FALLING /*IRQF_TRIGGER_FALLING*/, "100ask_mtp", ts_dev);
...
static int mtp_ft5x06_valid(struct i2c_client *client)
{
	...
	if(buf[0]  != 0x55)
	{
		printk("There is not  real device\n");
		return -1;
	}
